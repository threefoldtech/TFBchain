...
daemonpkgs = ./cmd/tfbchaind
clientpkgs = ./cmd/tfbchainc
...
configpkg = github.com/threefoldtech/TFBchain/pkg/config
ldflagsversion = -X $(configpkg).rawVersion=$(fullversion)

stdoutput = $(GOPATH)/bin
daemonbin = $(stdoutput)/tfbchaind
clientbin = $(stdoutput)/tfbchainc
